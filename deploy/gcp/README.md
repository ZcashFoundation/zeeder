...
IMAGE=docker.io/zfnd/dnsseeder:v1.3.0 ./deploy-seeders.sh --roll
...
spec (image, env, startup script), so it doubles as drift repair. Prefer pinning
`IMAGE` by digest once an Artifact Registry pipeline exists.
