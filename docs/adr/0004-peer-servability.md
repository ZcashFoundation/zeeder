...
- Target-version preference improves bootstrap answers during the conservative post-activation confirmation window without changing which peers remain admissible.
...
- Before that confirmation, target-version peers fill each DNS response before previous-version fallback peers.
...
- 2026-07-27: DNS responses began preferring compiled-target peers while retaining the independently observed admission floor.
