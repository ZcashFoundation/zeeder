...
`ns1.zfnd.org` through `ns6.zfnd.org`. Each is a Container-Optimized OS VM in its
own region. Those hosts back the `mainnet.seeder.zfnd.org` and
`testnet.seeder.zfnd.org` delegations, so a botched roll can stop peer discovery
for a whole network.
...
The fleet is delivered from `deploy/gcp`. The committed `IMAGE` pin and the
startup-script template carry the deploy mechanism and its contracts; the
inventory is operator-held:

- `fleet.conf` is the inventory. It is operator-held: gitignored and never
  committed, with `fleet.conf.example` documenting its shape, so the public
...
- `IMAGE` is a single line holding the full image reference pinned by `sha256`
  digest. Publishing a release is the deploy decision; the release workflow
  commits the published digest here and then rolls, so this file's git log is the
  deploy audit trail and a revert of it is the rollback.
...
The desired image is the committed `IMAGE` pin; actual state is the GCP API.
`seeders.sh --audit` reports the drift between them. Rollback is a `git revert`
of `IMAGE` followed by a roll.
...
- Terraform or OpenTofu for this fleet: rejected. Terraform has no health-gated,
  ...
  organization's existing compute automation is imperative gcloud; its only
  established Terraform practice manages DNS records, not compute. Revisit if the
  fleet grows past roughly ten hosts, a second resource type appears, or a second
  regular operator needs push-button deploys.
