...
Zone keys are lowercase (`mainnet`, `testnet`). The environment source lowercases
key segments, so always write zone keys in lowercase to keep file and environment
config consistent.
