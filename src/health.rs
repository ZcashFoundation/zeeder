...
/// Drop slow or incomplete health probes instead of keeping one task and file
/// descriptor open per connection.
...
async fn serve_connection(
    ...
    let read = timeout(CONNECTION_IO_TIMEOUT, stream.read(&mut buffer))
        .await
        .map_err(|_| connection_timeout("read"))??;
    ...
    timeout(CONNECTION_IO_TIMEOUT, stream.write_all(response.as_bytes()))
        .await
        .map_err(|_| connection_timeout("write"))??;
    timeout(CONNECTION_IO_TIMEOUT, stream.flush())
        .await
        .map_err(|_| connection_timeout("flush"))?
}

fn connection_timeout(operation: &str) -> io::Error {
    io::Error::new(
        ErrorKind::TimedOut,
        format!("health connection {operation} timed out"),
    )
}
...
mod tests {
    ...
    #[test]
    fn http_response_does_not_fold_headers() {
        let response = http_response(200, "OK", "ok\n");
        assert!(
            !response.contains("\r\n "),
            "headers must not start with obsolete folded whitespace"
        );
    }
}
